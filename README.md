...
以下定义来自原项目接线文档，按主机正面观察 RJ45，从左到右为 1~8 脚：
...
- `ESP32-C3 GPIO3` -> RJ45 `5`（PTT）
- `ESP32-C3 GPIO10` -> RJ45 `2`（COR）
- `ESP32-C3 GND` -> RJ45 `6`（GND）

音频建议：

- 电台音频输出（RJ45 `8`）-> 声卡 `MIC IN`
- 声卡输出 -> 电台麦克风输入（RJ45 `3`，地接 RJ45 `4/6`）

安全注意：

- RJ45 `7` 为 `+8V`，不要误接到 ESP32 GPIO。
- COR/PTT 建议加限流或隔离电路（电阻/三极管/光耦）。
- 上电前先用万用表复核地线和电压。
